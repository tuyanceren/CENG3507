...
class Course {
  ...
  getElement() {
    ...
    deleteStudentButton.innerText= 'Delete Student';

    const updateStudent= document.createElement('button');
    updateStudent.innerText= 'Update Student'
    ...
    courseInfo.appendChild(deleteStudentButton);
    courseInfo.appendChild(updateStudent);
    ...
    deleteStudentButton.addEventListener('click',(e)=>this.deleteStudent(e));
    updateStudent.addEventListener('click',(e)=>this.updateStudent(e));
  ...
  deleteStudent(e){
    ...
    const idInputd = document.getElementById('student_id');
    const nameInputd = document.getElementById('student_name');
    const surnameInputd = document.getElementById('student_surname');
    const midtermInputd = document.getElementById('student_midterm');
    const finalInputd = document.getElementById('student_final');

    const id = idInputd.value;
    const name = nameInputd.value;
    const surname = surnameInputd.value;
    const midterm = midtermInputd.value;
    const final = finalInputd.value;
    const avg = (midterm *0.4 + final*0.6)
    const letterGrade = this.calculateLetterGrade(this.scale,avg);
    const statu = letterGrade== "F" ? "FAILED" : "PASSED";
    const existingStudent = this.students.some((student) => student.id == id) ;
  ...
  updateStudent(e){
    ...
    const idInputu = document.getElementById('student_id');
    const nameInputu = document.getElementById('student_name');
    const surnameInputu = document.getElementById('student_surname');
    const midtermInputu = document.getElementById('student_midterm');
    const finalInputu= document.getElementById('student_final');

    const id = idInputu.value;
    const name = nameInputu.value;
    const surname = surnameInputu.value;
    const midterm = midtermInputu.value;
    const final = finalInputu.value;
    const avg = (midterm *0.4 + final*0.6)
    const letterGrade = this.calculateLetterGrade(this.scale,avg);
    const statu = letterGrade== "F" ? "FAILED" : "PASSED";
    const existingStudent = this.students.some((student) => student.id == id) ;
    ...
    if (existingStudent) {
      const student = new Student(id, name, surname, midterm, final,avg,statu,letterGrade);
      this.students.pop(student);
      const studentupdate = new Student(id, name, surname, midterm, final,avg,statu,letterGrade);
      this.students.push(studentupdate);
      alert("the student is update.");
    } else {
      alert("Student not found.");
  ...
  showFailed(){
    ...
    alert("failed student count:" + fail);
  }
  showPassed(){
    ...
    alert("passed student count:" + succes);
  }
}
